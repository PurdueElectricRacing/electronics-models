...
const DATA_FILE = joinpath(@__DIR__, "..", "datasets", "shockpots.csv")
const OUTPUT_FILE = joinpath(@__DIR__, "..", "figures", "shock_pot_error.png")
...
p = plot(
    ...
    size=(1000, 700),
    dpi=600,
    plot_title="Shock potentiometer error",
    left_margin=10mm,
    legend=false,
)
...
for (i, col) in enumerate(sensors)
    scatter!(
        ...
        subplot=i,
        title=header[col],
        xlabel=i == length(sensors) ? header[1] : "",
        ylabel="error (mm)",
        markersize=4,
        markerstrokewidth=0,
    )
